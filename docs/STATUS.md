...
| 항목 | 웹·Android | iOS |
| --- | --- | --- |
| 피드(견적 후기) + 후기 상세 | 있음 | **없음** (탭이 "준비중" 알림) |
| 자랑하기 `/brag` + 상세 모달 | 있음 | **없음** |
| 배우자 귀속(`BoundRoomRedirect`) | 있음 | 없음 |
| 가이드 오버레이 | 있음 | 없음 (머리 면의 `?` 자리 비움) |
| 초대 띠(`SoloPlanBanner`) | 있음 | 없음 |
| 프로필 `/user` 2열·라벨 규칙 | 있음 | 옛 모양 |
| 참여 플랜 카드 시각 언어 | 있음 | 옛 모양 |
| 캘린더 보드(월별 컬럼) | 넓은 화면 전용 | 해당 없음(폰 전용 앱) |

**눌러도 아무 일이 없는 버튼을 미리 두지 않는다** — 위 항목들은 부품이 준비될
때 화면과 함께 붙인다.
...
3. **홈 정렬 기본값이 최신순이라 지난 일정이 목록 맨 아래에 묻혔다.**
   폰 홈에는 정렬 버튼이 없어 사실상 고정값이다 — 다가오는 순이 맞다.
4. **홈 목록이 데이터를 받고도 앞 두 장이 뼈대로 남았다.** `LazyVStack` 이
   이미 그린 행을 위치로 재사용했다. 목록 `id` 를 loading 에 묶어 고쳤다.
