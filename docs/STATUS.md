...
Core 로직과 주요 화면 10개가 웹과 1:1 로 맞춰져 있고, 남은 건 큰 화면 2개 + 푸시다.

- 저장소: <https://github.com/Junnnnnnnnnnn/wedding-plant-ios> (**공개**)
- Core 테스트: **169개 통과** (Windows/Linux)
...
가장 큰 남은 화면(안드로이드 30K). 소켓 재연결 규칙이 까다롭다 —
무제한 재연결 + 지수 백오프 + 화면 복귀 시 재연결 + REST 로 재동기화.
`createDate` 는 **UTC** 라 그대로 쓰면 9시간이 어긋난다 (`KstInstant` 사용).
