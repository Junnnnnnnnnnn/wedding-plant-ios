...
Core 로직과 주요 화면 6개가 웹과 1:1 로 맞춰져 있고, 남은 건 큰 화면 5개 + 푸시다.

- 저장소: <https://github.com/Junnnnnnnnnnn/wedding-plant-ios> (**공개**)
- Core 테스트: **129개 통과** (Windows/Linux)
...
| 일정 추가/수정 | `/add-plen` | **없음** (다음 작업) |
| 예산 상세 | `/budget-detail` | **없음** |
| 캘린더 | `/calendar` | **없음** |
| 채팅 | `/chat/[id]` | **없음** |
| 공유 참여 | `/share/[code]` | **없음** |
...
- 메인의 **추가** 버튼 → `/add-plen` 필요
- 메인의 **도움말(?)** → 가이드 오버레이
- 메인의 **초대** 버튼 → 공유 링크 생성
- 메인의 **캘린더** 아이콘 → `/calendar`
- 상세의 **수정하기** → "준비 중" 안내만. 추가 화면과 같이 만든다

---

## 다음에 할 일 (우선순위)

### 1. 일정 추가/수정 (`/add-plen`) — 가장 값이 크다

메인의 "추가"와 상세의 "수정하기"가 동시에 살아난다. 안드로이드 `ui/addplan/` 48K.

주의할 점(전부 `IOS_PORTING_NOTES` 에 있음):

- **`POST /plan/schedule` 에 `roomId` 필수.** 빼면 200 인데 목록에 영영 안 나온다
- 수정은 `roomId` 를 **붙이지 않는다**
- 날짜를 미정으로 되돌릴 때만 `startDate: null` 을 명시 전송 (이미 `ScheduleWriteRequest` 가 처리)
- 게스트는 **최대 3개**, 로컬 id 는 음수 (`GuestStore` 에 구현됨)
- 카테고리 목록 경로가 상황마다 다르다 — 방 있음 / 로그인 / **비로그인(인증 없이)**
- 카테고리 추천은 `PlanRules.suggestCategories` (Core 에 구현·테스트됨)
- 카테고리 칩 색은 `categoryPastelHex`(8색). 리스트의 `categoryColorHex`(6색)와 **다른 팔레트**다
- **장소 검색 API 는 아직 백엔드에 없다**(404). 안내 문구로 처리하고, 생기면 그대로 붙게 만들 것
...
예산 상세(차트) → 캘린더 → 채팅 순. 채팅은 소켓 재연결 규칙이 까다롭다
(무제한 재연결 + 지수 백오프 + 화면 복귀 시 재연결 + REST 로 재동기화).
