...
Core 로직과 주요 화면 9개가 웹과 1:1 로 맞춰져 있고, 남은 건 큰 화면 3개 + 푸시다.

- 저장소: <https://github.com/Junnnnnnnnnnn/wedding-plant-ios> (**공개**)
- Core 테스트: **149개 통과** (Windows/Linux)
...
메인의 캘린더 아이콘이 살아난다. 안드로이드 `ui/calendar/` 21K.
날짜 계산은 전부 `KstDate` 로 한다 (`new Date("YYYY-MM-DD")` 류의 파싱 금지).
...
채팅(`/chat/[id]`, 30K) → 공유 참여(`/share/[code]`, 7K).
채팅은 소켓 재연결 규칙이 까다롭다
