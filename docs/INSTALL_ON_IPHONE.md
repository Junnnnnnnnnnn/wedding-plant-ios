...
**이걸 먼저 하지 않으면 앱을 깔아도 아무도 못 들어온다.** 게스트 모드가 없어서
로그인 말고는 들어올 문이 없고, 키가 비면 로그인 버튼이 설정 오류만 낸다.
그래서 **Release 빌드는 키가 없으면 아예 실패한다** (`project.yml` 의
`카카오 앱 키 확인 (Release)` 스크립트).

키는 카카오 개발자 콘솔의 **앱 키 > 네이티브 앱 키**다 — REST API 키·JS 키가 아니다.
안드로이드와 **같은 앱, 같은 키**를 쓴다(`wedding-plant-android/local.properties` 에 있다).

```powershell
gh secret set KAKAO_NATIVE_APP_KEY --body "<네이티브 앱 키>"
```

로컬(Xcode)에서는 `Config/Local.xcconfig` 에 적는다 (gitignore 됨).

### 콘솔에 iOS 플랫폼을 등록해야 한다

**키를 넣는 것만으로는 안 된다.** 카카오는 플랫폼별로 따로 등록을 받고,
등록되지 않은 번들 ID 로 로그인하면 거절한다. 안드로이드가 되는 것과 무관하다.

1. <https://developers.kakao.com> → 내 애플리케이션 → 웨딩플랜 앱
2. **앱 설정 > 플랫폼 > iOS 등록**
   - 번들 ID: `com.zipshowkorea.weddingplant` (안드로이드 패키지명과 같다)
   - 마켓 URL 은 출시 전이라 비워 둬도 된다
3. **제품 설정 > 카카오 로그인** 이 켜져 있는지 확인 (안드로이드용으로 이미 켜져 있다)

> iOS 는 안드로이드와 달리 **키 해시가 필요 없다.** 번들 ID 하나면 된다.
